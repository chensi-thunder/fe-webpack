...
class FeprojectBaseInfoPlugin {
  ...
  apply (compiler) {
    ...
    if (this._parseStdout('git rev-parse --is-inside-work-tree') !== 'true') {
      shell.echo(`${PLUGIN_NAME} 插件通过 git 命令获取版本信息，当前路径为非 git 仓库路径`)
      return
    }

    // 获取 packageJson
    pkgJson = require('safe-require')(`${compiler.options.context}/package.json`)
  ...
  onBeforeEmit (data, cb = null) {
    ...
    html = html.replace('</body>',
      `<script type="text/javascript">window.fe = ${JSON.stringify(projectBaseInfo)}</script></body>`)
